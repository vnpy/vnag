...
### 文档处理优化
- **固定分块**: 1000字符分块，200字符重叠保证上下文连续性
- **Markdown优化**: 按标题层级(#, ##, ###)优先分块，避免语义截断
- **语义保护**: 保留文档原始语义结构，避免句子和代码块中间截断
...
### API异常处理
- 捕获APIError、AuthenticationError、RateLimitError等具体错误
- 返回明确的、可被UI理解的错误信息
- 在聊天界面显示具体错误原因

### Token监控
- 简单字符数估算（中文约1.5字符/token，英文约4字符/token）
- 80%使用量时在界面显示黄色提示

## 开发路线

### 第一阶段（MVP核心功能）
- 基础RAG问答 + 简单聊天界面 + 多格式文档支持
- 配置管理 + 输入验证 + API异常处理
