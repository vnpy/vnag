...
> **安全说明**：文件系统工具的使用受到权限控制。您需要在 `.vnag/tool_filesystem.json` 配置文件中明确指定允许读写的路径，以防止 Agent 访问未授权的文件。
...
**A:** 出于安全考虑，文件系统工具需要明确的权限配置。请在 `.vnag/tool_filesystem.json` 文件中的 `read_allowed` 或 `write_allowed` 数组中添加您需要访问的目录路径。配置文件会在首次加载工具时自动生成。
